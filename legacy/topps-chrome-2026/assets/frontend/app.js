...
function poolFor(setName){
  const row=data.odds.find(r=>r.name===setName);
  return data.sections[row?.section] || basePool;
}
...
function isAuto(name){return data.odds.find(r=>r.name===name)?.auto || false}
...
function generateBox(){
  ...
  if(packs.flat().filter(c=>!c.denom).length < Math.max(0,cfg.guaranteedAutos-autos)) return generateBox();
...
function detailHtml(c){return `<div class="detail-accent" style="--accent:${c.color}"></div><div><b>${escapeHtml(c.name)} ${badges(c)}</b><span>${escapeHtml(c.team||'球隊未列')} · #${escapeHtml(c.code||'—')}</span></div><div class="detail-set"><b>${escapeHtml(c.set)}</b><span>${c.denom?`官方逐包機率 1:${c.denom.toLocaleString()}`:'基本卡'}${c.auto?' · AUTO 簽名卡':''}${c.forced?' · 整盒保證補足':''}</span></div>`}
...
async function openPack(showDialog=true){
  if(busy||box.index>=CONFIG[format].packs)return;busy=true;lockControls(true);
  ...
  await new Promise(r=>setTimeout(r,Math.min(1200,380+pack.length*90)));busy=false;lockControls(false);
  if(box.index>=CONFIG[format].packs&&showDialog)finish();
}
function lockControls(value){document.querySelectorAll('#formats button,#newBox,#newBoxTop,#openPack,#openAll,#packButton').forEach(b=>b.disabled=value)}
function openAll(){if(busy||box.index>=CONFIG[format].packs)return; const rest=box.packs.slice(box.index);box.opened.push(...rest);box.hits.push(...rest.flat().filter(c=>c.denom));box.index=CONFIG[format].packs;$('#packButton').style.visibility='hidden';renderProgress();renderSummary();renderHistory();finish()}
...
function newBox(){if(busy)return;format=format in CONFIG?format:'hobby';box=generateBox();renderFormats();renderSpec();renderProgress();renderSummary();renderHistory();resetStage()}
function finish(){renderCards(box.opened.flat().sort((a,b)=>(b.denom||0)-(a.denom||0)),true);$('#hint').textContent='整盒結果：依官方 1:X 由高到低排列（最稀有 → Base）';const rare=box.hits.filter(c=>c.denom>=1000).length,autos=box.hits.filter(c=>c.auto).length;$('#finishStats').innerHTML=`共開出 <b>${box.hits.length}</b> 張特殊卡<br><b>${rare}</b> 張稀有以上 · <b>${autos}</b> 張簽名卡`;$('#finishDialog').showModal()}

$('#openPack').onclick=async()=>{if(busy||box.index>=CONFIG[format].packs)return;if(box.index>0)resetStage();await openPack()};
